...
ARG     TOR_PROXY_HOST
ARG     TOR_PROXY_PORT
RUN     apt-get update && apt-get install -y squid telnet openssh-server curl net-tools netcat && apt-get clean
...
RUN     echo "cache_peer ${TOR_PROXY_HOST} parent ${TOR_PROXY_PORT} 7 proxy-only default" >> /etc/squid/main.squid.conf
RUN     echo 'http_access allow all' >> /etc/squid/main.squid.conf
RUN     echo 'never_direct allow all' >> /etc/squid/main.squid.conf
RUN     echo '# - from dockerfile -' >> /etc/squid/main.squid.conf
RUN     echo 'PermitRootLogin no' >> /etc/ssh/sshd_config
RUN     echo '#!/bin/bash' > /bin/tunproxy_run.sh && chmod 700 /bin/tunproxy_run.sh
